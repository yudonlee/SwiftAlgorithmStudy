...
let FIO = FileIO()
let N = FIO.readInt() //집(정점) 개수
let M = FIO.readInt() //길(간선) 개수
...
for _ in 0..<M {
    let A = FIO.readInt() //src
    let B = FIO.readInt() //dest
    let C = FIO.readInt() //cost
    
    pq.push(Edge(weight: C, src: A, dest: B))
}
