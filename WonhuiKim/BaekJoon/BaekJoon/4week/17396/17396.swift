...
let FIO = FileIO()
let N = FIO.readInt() //분기점(정점) 수
let M = FIO.readInt() //길(간선) 수

var available = [Int]()
for _ in 0..<N {
    available.append(FIO.readInt())
}
available[N-1] = 0
...
var dist: [Int] = Array(repeating: maxDist, count: N)
    ...
    if let now = pq.pop() {
        if dist[now.1] == maxDist {
            ...
            for next in edges[now.1] {
                if dist[next.dest] == maxDist && available[next.dest] != 1{
