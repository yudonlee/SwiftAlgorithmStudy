//
//  1753.swift 최단경로(Dijkstra)
//  BaekJoon
//
//  Created by 김원희 on 2022/05/28.
//
...
let V = FIO.readInt() //정점 개수
let E = FIO.readInt() //간선 개수
let start = FIO.readInt() //시작 정점
...
for _ in 0..<E {
    let src = FIO.readInt() //시작
    let dest = FIO.readInt() //끝
    let weight = FIO.readInt() //가중치
...
var dist: [Int] = Array(repeating: maxDist, count: V+1)
